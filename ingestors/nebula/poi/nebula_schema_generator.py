#!/usr/bin/env python3
"""
Nebula Graph Schema Generator for POI Ontology

Converts the POI ontology to Nebula Graph schema definitions.
This script is part of the Nebula ingestion process.
"""
...
class NebulaSchemaGenerator:
    ...
    def generate_schema(self) -> str:
        ...
        # Add header
        schema_parts.append("-- Nebula Graph Schema for POI Ontology")
        schema_parts.append(f"-- Generated from: {self.ontology_path}")
        schema_parts.append(f"-- Generated at: {datetime.now().isoformat()}")
        schema_parts.append("")
        
        # Add space creation
        schema_parts.append("-- Create space for POI data")
        schema_parts.append("CREATE SPACE poi_space (partition_num = 10, replica_factor = 1, vid_type = FIXED_STRING(32));")
        schema_parts.append("USE poi_space;")
        schema_parts.append("")
    ...
    def save_timestamped_schema(self, schema_dir: str = "schema"):
        ...
        filename = f"nebula_schema_{timestamp}.ngql"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate Nebula Graph schema from POI ontology')
    parser.add_argument('--ontology', default='../../../ontology/poi/poi_ontology.yaml', 
                       help='Path to ontology YAML file')
    parser.add_argument('--output', default='schema/nebula_schema.ngql',
